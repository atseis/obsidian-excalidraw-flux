...
- Arrowhead shortcuts: `h` opens the native Arrowhead picker as a persistent, fully expanded palette; choices can be tried repeatedly until `Esc` closes it. The complete mapped set is `q` None, `w` Arrow, `e` Triangle, `r` Triangle outline, `t` Chevron, `y` Chevron outline, `b` Block arrow, `n` Block arrow outline, `a` Circle, `s` Circle outline, `d` Diamond, `f` Diamond outline, `z` Bar, `x` Cardinality one, `c` Cardinality many, `v` Cardinality one-or-many, `i` Cardinality exactly one, `o` Cardinality zero-or-one and `m` Cardinality zero-or-many. Lowercase keys update the existing endpoint of a one-way arrow (normally `arrowhead_end`, but `arrowhead_start` for a start-only arrow); uppercase keys always update `arrowhead_start`. `hh` copies the existing one-way Arrowhead to the empty endpoint, while `hH` clears `arrowhead_start` only when both endpoints are populated. Empty and already-applicable states remain unchanged. In Flux host mode, `H` belongs exclusively to this palette and no longer activates Hand; Hand remains available from the toolbar, Space-drag and middle mouse. Standalone Excalidraw without host shortcut preferences retains its native `H` Hand alias.
...
- The Custom `Reverse arrow direction.md` script lives under `plugins/Excalidraw/Scripts/Downloaded(Custom)/`; the official downloaded `Downloaded/Reverse arrows.md` remains untouched. The Custom script reverses the selected arrow's point order and local origin, swaps endpoint bindings, reverses Elbow fixed-segment metadata and start/end special flags, and deliberately preserves `startArrowhead`, `endArrowhead`, stroke style, roundness and `customData`. This keeps the drawn path visually fixed while reversing the actual traversal and line-animation direction. The `script.line.reverse-arrows` Action and `Alt+R` binding target this Custom command.
...
The current deployed build is `2.27.0-flux.20260817.11` in both `/home/wy/文档/obsidian-test` and HyperFlux. Plugin deployment replaced only `main.js`, `styles.css` and `manifest.json`; both vaults retained their existing plugin `data.json`, and HyperFlux retained its community-plugin list. The only intentional HyperFlux hotkey migration removed `Alt+R` from the obsolete official/YMJR Reverse arrows commands and assigned it to `obsidian-excalidraw-plugin:Downloaded(Custom)/Reverse arrow direction`. The Obsidian CLI could not connect to the already-running main process, so loading the copied runtime still requires a manual **Reload app**.

- Core regression suite: 6 focused files, 212/212 tests passed, including Block arrow geometry across every path, Automatic Curve endpoint-marker direction, all 19 lowercase/adaptive and 19 uppercase/start Arrowhead mappings, the persistent fully expanded Arrowhead palette, `hh`/`hH` directionality actions, Connection Mode and Arrow Type selectors, persistent Bucket Fill palette behavior, next-arrow defaults, Points-mode start snapping, legacy animation/brace formats, font compatibility, configurable 0–8 tool aliases, the 30-slot color palette and all keyboard-input guards. The separate Hand lookup gate remains covered for host and standalone modes. The `ls`/`ld`/`lt` cases also start with an animated line and prove that the animation field is removed without deleting unrelated custom data.
- Core type checking, Core `build:obsidian` and the plugin production build completed successfully. Core commit `98ae72f151194fd5912f3bb0e5d01b5dfe9cf525` is the release-workflow pin. The deployed `main.js`, built `dist/main.js` and both source manifests identify `2.27.0-flux.20260817.11`. Both vault deployments and `dist` match exactly: `main.js` SHA-256 `4b8e52f3337d672706e26ac60988706b73817bb8a38705b75448842f4f47031f`, `styles.css` `40fab07c21a6d753728b9032fe0dc552e97404a6e4ac550202fdc70bbf6f9cde`, and `manifest.json` `b6dc1f78744a3fd2d96f92e5dff8dc97c7c778ceda5c87c60f6940cad528b5c7`. The deployed `main.js` is 4,833,071 bytes.
...
- The persistent Arrowhead palette changes selected arrows and the next-arrow start/end defaults without changing Sharp, Round, Elbow or Automatic Curve path data. An Automatic Curve probe applied lowercase Block arrow and uppercase Block arrow outline choices, producing `endArrowhead: "block_arrow"` and `startArrowhead: "block_arrow_outline"` while preserving `customData.curveArrow`, its animation metadata, `roundness` and `elbowed`. A start-only one-way arrow proves lowercase follows its populated start endpoint, while ordinary one-way arrows continue to use the end.
- The Custom geometry-reversal script passed one-step and involution probes: it reverses world-space traversal while preserving the visible path, Arrowhead assignments, stroke style, Automatic Curve/animation metadata and roundness; it swaps bindings and correctly reverses Elbow fixed-segment indices, endpoints and special flags. Applying it twice restores every checked field exactly. The old downloaded Reverse arrows script remains byte-for-byte unchanged.
...
- Of the 69 script-backed Actions, 68 retain their original script path and behavior. `script.line.reverse-arrows` deliberately keeps its stable Action ID but now targets `plugins/Excalidraw/Scripts/Downloaded(Custom)/Reverse arrow direction.md`; its old downloaded source remains present and unchanged for reference.
...
- Excluding Action UI state and HyperFlux-only generated Reports/template WebP exports, the test and HyperFlux `plugins/Excalidraw` resource trees match after synchronization. Both contain the same Custom reverse script and updated stable-ID Action; all 26 `configs/plugins/Excalidraw` files remain aligned.
