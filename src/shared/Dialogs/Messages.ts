...
- Added two-stroke selectors for Connection Mode (\`co\` Off, \`cp\` Points, \`ce\` Edges) and Arrow Type (\`as\` Sharp, \`ac\` Curved, \`ae\` Elbow, \`aa\` Automatic Curve).
- \`H\` now opens a persistent, fully expanded Arrowhead palette. Lowercase keys update the existing one-way endpoint (normally the end), uppercase keys explicitly update the start, \`hh\` makes a one-way arrow bidirectional, \`hH\` makes a bidirectional arrow end-only, and \`Esc\` closes the palette. All Cardinality Arrowheads now have mnemonic keys. In Flux, \`H\` remains reserved for Arrowheads; Hand is available from the toolbar, Space-drag and the middle mouse button.
- Added the **Reverse arrow direction** Custom script. It reverses the selected arrow's geometry, bindings, Elbow segments and animation direction while preserving its Arrowhead assignments and path shape; it is intended for \`Alt+R\` from \`Scripts/Downloaded(Custom)\`.
- Stroke, background and Bucket Fill now share a 30-slot full-spectrum palette. The added keys are Coral (\`h\`), Amber (\`j\`), Chartreuse (\`k\`), Forest (\`u\`), Turquoise (\`y\`), Azure (\`[\`), Indigo (\`]\`), Lavender (\`;\`), Magenta (\`'\`) and Burgundy (\`,\`); every family retains Shift+1…5 shade controls.
- \`B\` activates Bucket Fill and opens the full background palette as a persistent paint session: colors and shades can be changed repeatedly, multiple regions can be filled, and \`Esc\` closes the palette. The official standalone build retains its original repeated-\`B\` color cycling behavior.
