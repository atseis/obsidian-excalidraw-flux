...
- \`H\` now opens the Arrowhead selector using the native picker letters. A lowercase second key changes the end Arrowhead; Shift+the second key changes the start. Examples: \`hq\` = None, \`hw\` = Arrow, \`hb\` = Block arrow, and \`hn\` = Block arrow outline. In Flux, \`H\` is reserved for Arrowheads and no longer activates Hand; Hand remains available from the toolbar, Space-drag and the middle mouse button.
